...
const PetId = ({ setStep, updatePetData, petData }: PetIdProps) => {
  ...
  const getAllItems = (): BreedItem[] => {
    ...
    return breeds.filter(
      (breed): breed is BreedItem =>
        breed.id !== undefined && breed.name !== undefined
    );
  };
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length > 0) {
      ...
      const filtered = allItems.filter((item) =>
        item.name.replace(/\s+/g, "").includes(value)
      );
  ...
  const handleDropDownClick = (value: string) => {
    ...
    const selectedBreed = filteredItems.find(
      (breed) => breed.name.replace(/\s+/g, "") === trimmedValue
    );
    if (selectedBreed) {
  ...
  const isInputValid = filteredItems.some(
    (breed) => breed.name.replace(/\s+/g, "") === input
  );
